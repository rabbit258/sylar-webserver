...
Address::ptr Address::Create(const sockaddr *addr, socklen_t addrlen)
{
    ...
    switch(addr->sa_family){
        ...
        case AF_INET6:
            ...
            break;
        default:
            result.reset(new UnkownAddress(*(const sockaddr*)addr));
            break;
    }
    return result;
}
...
bool Address::Lookup(std::vector<Address::ptr> &result, const std::string &host, int family, int type, int protocol)
{
    ...
    if(error){
        SYLAR_LOG_ERROR(g_logger) << "Address::LookUp getAddrss(" << host << ","
...
bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t>> &result, int family)
{
    ...
    try{
        ...
        SYLAR_LOG_ERROR(g_logger) << "Addres::GetInterfaceAddress exceptio";
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
IPAddress::ptr IPv4Address::newwordAddress(uint32_t prefix_len)
{
    if(prefix_len > 32){
        return nullptr;
    }
...
IPv6Address::ptr IPv6Address::Creater(const char *address, uint16_t port)
{
...
{
    os << "[";
    uint16_t * addr = (uint16_t *)m_addr.sin6_addr.s6_addr16;
...
IPAddress::ptr IPv6Address::newwordAddress(uint32_t prefix_len)
{
    sockaddr_in6 baddr(m_addr);
    baddr.sin6_addr.s6_addr[prefix_len/8] &=
        CreateMask<uint8_t>(prefix_len % 8);

    for(size_t i = 0;i<prefix_len/8-1;++i){
