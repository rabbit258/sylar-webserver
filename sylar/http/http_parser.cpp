...
static sylar::ConfigVar<uint64_t>::ptr g_http_request_buff_size = 
...
static uint64_t s_http_request_buffer_size = g_http_request_buff_size->getValue();
static uint64_t s_http_request_max_body_size = g_http_request_max_body_size->getValue();

namespace {
struct _RequestSizeIniter {
    _RequestSizeIniter() {
        s_http_request_buffer_size = g_http_request_buff_size->getValue();
        s_http_request_max_body_size = g_http_request_max_body_size->getValue();
        g_http_request_buff_size->addListener(
...
{
    SYLAR_LOG_DEBUG(g_logger) << std::string(data,len);
    size_t offset = http_parser_execute(&m_parser,data,len,0);
    if(offset == (size_t)-1) {
        SYLAR_LOG_WARN(g_logger) << "invalid request: " <<std::string(data,len);
    } else if(offset != 1){ // not finish yet
...
uint64_t HttpRequestParser::getContentLenth()
{
...
void on_response_reson(void *data,const char *at,size_t len){
    HttpResponseParser* parser = static_cast<HttpResponseParser*>(data);
    parser->getData()->setReason(std::string(at, len));
} 
...
void on_response_chunk(void *data,const char *at,size_t len){
...
void on_response_header_done(void *data,const char *at,size_t len){
    
} 
void on_response_last_chunk(void *data,const char *at,size_t len){
...
size_t HttpResponseParser::execute(char *data, size_t len)
{
    ...
    if(offset == (size_t)-1) {
        SYLAR_LOG_WARN(g_logger) << "invalid response: " <<std::string(data,len);
    } else if(offset != 1){ // not finish yet
        memmove(data, data  + offset ,(len - offset));
    }
    return offset;
...
uint64_t HttpResponseParser::getContentLenth()
{
    return m_data->getHeaderAs<uint64_t>("content-length",0);
}
}
