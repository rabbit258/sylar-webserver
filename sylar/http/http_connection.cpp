...
HttpResponse::ptr HttpConnection::recvResponse()
{
    ...
    if(client_parser.chunked) {
        ...
        int64_t length = parser->getContentLenth();
        if(length > 0){
            std::string body;
            body.resize(length);
...
HttpResult::ptr HttpConnection::DoGet(Uri::ptr uri
                                    , uint64_t timeout_ms, const std::map<std::string
                                    , std::string> &headers
                                    , const std::string &body)
{
                                        ...
                                        , uint64_t timeout_ms)
{
    ...
    if(!addr) {
        return std::make_shared<HttpResult>((int)HttpResult::Error::INVAILD_HOST
                    ,nullptr, "invaild host: "+ uri->getHost());
    }
    Socket::ptr sock = Socket::CreateTCPSocket(addr);
    if(!sock){
        return std::make_shared<HttpResult>((int)HttpResult::Error::CREATE_SOCKET_FAIL
    ...
    if(!rsp){
        return std::make_shared<HttpResult>((int)HttpResult::Error::TIMEOUT
            , nullptr, "recv response timeout" + addr->toString() + " timeouts_ms: " + std::to_string(timeout_ms));
    }
...
HttpConnection::ptr HttpConnectionPool::getConnection()
{
    ...
    if(!ptr){
        ...
        Socket::ptr sock = Socket::CreateTCPSocket(addr);
        if(!sock){
            SYLAR_LOG_ERROR(g_logger) << "create sock fail: " << m_host;
            return nullptr; 
        }
        if(!sock->connect(addr)){
            SYLAR_LOG_ERROR(g_logger) << "socket connect fail: " << m_host;
            return nullptr; 
        }
                                            ...
                                            , uint64_t timeout_ms)
{
    ...
    if(!rsp){
        return std::make_shared<HttpResult>((int)HttpResult::Error::TIMEOUT
            , nullptr, "recv response timeout" + sock->getRemoteAddress()->toString() + " timeouts_ms: " + std::to_string(timeout_ms));
    }
    return std::make_shared<HttpResult>((int)HttpResult::Error::OK, rsp ,"OK");
