...
{
    if(size > getReadSize()){
        // SYLAR_LOG_ERROR(g_logger) << "need read "<< size << "but only has "<< getReadSize() <<" read buff";
...
void ByteArray::read(void *buf, size_t size, size_t position) const
{
    ...
    while(size >0){
        if(ncap >= size){
            memcpy((char *)buf + bpos, (const char *)cur->ptr + npos, size);
            ...
            memcpy((char *)buf + bpos,(const char *)cur->ptr + npos,ncap);
            position += ncap;
            bpos += ncap;
            size -= ncap;
            cur = cur->next;
